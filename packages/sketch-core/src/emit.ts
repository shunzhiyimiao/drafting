 ...
 *  the codegen-server's pinned Prettier is the final formatter. */
export function toJsxString(sketch: Sketch, theme: Theme): string {
  ...
  return [
    ...
    `// Regenerated wholesale from sketches/${sketch.name}.sketch.json (data-sk = node id).`,
    ``,
    handlersType,
    ``,
